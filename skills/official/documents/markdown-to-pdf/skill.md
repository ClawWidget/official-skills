...
task: Drop a Markdown file to convert it into a PDF using Pandoc (local-only).
icon: file-output
llm-hint: "Fires on file drop. Converts a local Markdown file (.md) to PDF using Pandoc and writes <input>.pdf beside the source file. Returns {saved_path}. Local-only, no network. Useful as the document-render step after an LLM rewrite skill that outputs Markdown."
