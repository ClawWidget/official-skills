...
task: Drop PDF files to extract plain text via Pandoc. Local-only, no network.
icon: file-text
llm-hint: "Fires on file drop. Extracts plain text from PDFs using Pandoc (--to plain). Returns {text}. Local-only — no LLM, no network. Requires pandoc sidecar. Chain text output into an LLM step for analysis or summarization."
