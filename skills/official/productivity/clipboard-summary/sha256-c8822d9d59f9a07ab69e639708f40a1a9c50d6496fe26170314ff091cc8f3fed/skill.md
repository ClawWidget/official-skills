...
icon: clipboard-list
llm-hint: "Fires when text is copied. Summarizes copied or pasted text to 2-3 bullets via LLM, then fires official/utilities/notify (auto-installed via requires). Returns {summary}. Requires LLM API key in Keychain. First real requires-chain in the catalog — install this skill and official/utilities/notify is fetched automatically from the registry."
