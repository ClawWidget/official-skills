...
icon: globe
llm-hint: "Fires when text is copied. Translates copied or pasted text to clear, natural English via LLM (if already English, light polish only), shows the result in a notification, then calls official/utilities/notify (JIT via requires). Returns {english_text}. Requires LLM API key; network is llm-only. Same chain shape as official/productivity/clipboard-summary and clipboard-proofread."
