...
task: Drop video files to convert them into GIFs for easy sharing.
icon: film
llm-hint: "Fires on file drop. Converts local video files (mp4/mov/mkv/webm) to GIFs using ffmpeg and writes <input>.gif beside each source file. Returns {saved_path}. Local-only, no network. Use for simple clip sharing when animation format is preferred."
