...
task: Drop a video file to convert it into an .m4a audio file using ffmpeg.
icon: music
llm-hint: "Fires on file drop. Converts a local video file (mp4/mov/mkv/webm) to an .m4a file using ffmpeg and writes <input>.m4a beside the source file. Returns {saved_path}. Local-only, no network. Commonly used before official/media/transcribe-audio."
