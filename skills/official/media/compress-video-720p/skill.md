...
task: Drop a video file to generate a smaller MP4 share copy using ffmpeg.
icon: clapperboard
llm-hint: "Fires on file drop. Creates a share-friendly MP4 copy from a local video file using ffmpeg and writes <input>-720p.mp4 beside the source file. Returns {saved_path}. Local-only, no network. Use when compatibility and easier sharing matter more than preserving original encoding."
