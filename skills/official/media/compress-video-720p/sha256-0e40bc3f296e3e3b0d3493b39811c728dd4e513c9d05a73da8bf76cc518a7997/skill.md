...
task: Drop video files to generate smaller MP4 share copies using ffmpeg.
icon: clapperboard
llm-hint: "Fires on file drop. Creates share-friendly MP4 copies from local video files using ffmpeg and writes <input>-720p.mp4 beside each source file. Returns {saved_path}. Local-only, no network. Use when compatibility and easier sharing matter more than preserving original encoding."
