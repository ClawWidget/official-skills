...
icon: chart-line
llm-hint: "Fires on file drop. Uses DuckDB to compute a 7-row rolling average for a `revenue` column from dropped CSV files via read_csv_auto('$DROPPED_FILE'). Returns {result_json} with revenue and rolling_avg_7 rows in input order. Local-only, no network."
