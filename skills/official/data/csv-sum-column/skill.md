...
task: Drop a CSV file and compute SUM(revenue) locally with DuckDB.
icon: sigma
llm-hint: "Fires on file drop. Uses DuckDB to compute SUM(revenue) from a dropped CSV via read_csv_auto('$DROPPED_FILE'). Returns {result_json}. Local-only, no network. Use for quick revenue totals before optional LLM interpretation or notifications."
